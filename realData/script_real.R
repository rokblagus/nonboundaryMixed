##creates tables for the lmm, binom, pois, gamma examples as R objects res_lin, res_binom, res_pois and res_gamma
...
fit_glmer_r<-glmmTMB(Y~X-1 +
                     (Z-1 | grouping), data = xdf,family=Gamma(link="log"),REML=TRUE)

 
##using tau (code not done yet!)
fiter_gamma_tau<-function(tau,D_est,xdf){
  q<-ncol(D_est)
  ee<-eigen(D_est)
  ee$values[ee$values<1e-4]<-1e-4
  ee$values[ee$values>1e4]<-1e4
  ...
  d2222<-make_pseudo_data_rand_eigen_general_psi_v3_glmm(psi,nu,const=1e8,param="variance",link_fun = function(x) exp(x) )
  
  
  
  Ya<-c(Y,d2222$data$Y)
  #Ya<-Ya*100
  #offset<-rep(log(100),length(Ya))
  ...
  #xdfa<-list(Y=Ya,X=Xa,Z=Za,grouping=groupa,weights=weights,offset=offset)
  xdfa<-list(Y=Ya,X=Xa,Z=Za,grouping=groupa,weights=weights)
...
get_marLik_gamma<-function(fited_model,xdf){
  
  tmp2<-fited_model
  
  tmp3<-glmmTMB(Y~-1+X+(-1+Z|grouping),data=xdf,family=Gamma(link="log"),
...
tau_finder_gamma<-function(tau,xdf,D_est,fit_ml,alpha=0.05){
  fit_tau<-fiter_gamma_tau(tau,D_est,xdf)
  abs(get_marLik_gamma(fit_tau,xdf)-get_marLik_gamma(fit_ml,xdf))-qchisq(1-alpha,1)/2
}
...
for (i in sek){
  zz=zz+1
  y[zz]<-tau_finder_gamma(i,xdf,D,fit_glmer)
}
plot(sek,y,type="l")
abline(h=0)

opt_tau_ml<-uniroot(tau_finder_gamma,c(0,1),xdf=xdf,D_est=D,
                 fit_ml=fit_glmer,alpha=0.05)

fit_tau_ml<-fiter_gamma_tau(opt_tau_ml$root,D_est=D,xdf=xdf)
...
#for sd: we need 1/sqrt(exp(betad)) #do we need sqrt in the new release? use the same as for old release!

vr<-mod$sdr$cov.fixed[8,8]
est<-mod$sdr$par.fixed[8]
names(est)<-names(vr)<-"theta"

#s.r<-deltaMethod(est,"1/exp(theta)",vr) #ok for new release?
s.r<-deltaMethod(est,"1/sqrt(exp(theta))",vr) #old release
...
#for sd: we need 1/sqrt(exp(betad))

vr<-mod$sdr$cov.fixed[8,8]
est<-mod$sdr$par.fixed[8]
names(est)<-names(vr)<-"theta"

s.r<-deltaMethod(est,"1/sqrt(exp(theta))",vr) #old relase, 
#s.r<-deltaMethod(est,"1/exp(theta)",vr) #ok for new release?
...
#bml: is disp comparable to sigma? Tina says yes (numerous web sources would also seem to confirm this). can we get ses??
...
fixf<-res.bglmer[-c(1:3)]

th<-fit_bglmer@optinfo$val[1:3]*sigma(fit_bglmer)
h<-fit_bglmer@optinfo$derivs$Hessian
h<-solve(h)
v<-forceSymmetric(h+t(h))
v<-v[1:3,1:3]*sigma(fit_bglmer)**2
names(th)<-colnames(v)<-rownames(v)<-paste0("theta",1:3)

#the delta method doesnt work! here we assume that sigma(fit_bglmer) is constant, which is not true, but I dont see any other way...see below for what I tried 

s11<-deltaMethod(th,"sqrt(theta1*theta1)",v)
r12<-deltaMethod(th,"(theta1*theta2)/sqrt(  (theta1*theta1)*(theta2**2+theta3**2)  )   ",v)
s22<-deltaMethod(th,"sqrt(theta2**2+theta3**2)",v)

###this should work: here we rely on the results of https://www.sciencedirect.com/science/article/pii/S0167715222002048, where they derive the se for dispersion
m<-length(unique(aids$patient))
n<-mean(table(aids$patient))

phi<-sigma(fit_bglmer)**2
var.phi<-phi^4/(trigamma(1/phi)-phi)/m/n

th<-c(fit_bglmer@optinfo$val[1:3],phi) 
h<-fit_bglmer@optinfo$derivs$Hessian
h<-solve(h)
v<-forceSymmetric(h+t(h))
v<-v[1:3,1:3]
...
names(th)<-colnames(v)<-rownames(v)<-paste0("theta",1:4)

# transformation is theta1*sqrt(theta4)! 

s11<-deltaMethod(th,"sqrt(theta1*sqrt(theta4)*theta1*sqrt(theta4))",v)
r12<-deltaMethod(th,"(theta1*sqrt(theta4)*theta2*sqrt(theta4))/sqrt(  (theta1*sqrt(theta4)*theta1*sqrt(theta4))*((theta2*sqrt(theta4))**2+(theta3*sqrt(theta4))**2)  )   ",v)
s22<-deltaMethod(th,"sqrt((theta2*sqrt(theta4))**2+(theta3*sqrt(theta4))**2)",v)
...
#disp
d<-deltaMethod(th,"sqrt(theta4)",v)

disp<-paste0(round(d[1],3)," (",round(d[2],3),")" )

resb<-c(fixf,res.s3.bml,disp)

res<-cbind(resm,resm_reml,resb, resp2_ml )
colnames(res)<-c("ML","REML", "BM", paste0("PML(",round(opt_tau_ml$root,2),")"))
