    ...
    for (vr_int in c(0.01,0.1)){
      for (mult in c(0.5,1.5)){
        for (rh in c(0.5,0.8)){
          ...
          nms<-paste("bin",N,n,diag(corr_mat)[1],diag(corr_mat)[2],cov2cor(corr_mat)[1,2],sep="_")
          
          nmsfile<-paste0("bin/results/",meth,nms,".txt")
      ...
      for (mult in c(0.5,1.5)){
        for (rh in c(0.5,0.8)){
          mod="bin"
            ...
            nmsfile<-paste0("bin/results/taus",nms,".txt")
      ...
      for (mult in c(0.5,1.5)){
        for (rh in c(0.5,0.8)){
          ...
          for (meth in c("reml","bml", "tml","to", "o")){
            zz=zz+1
            dfi<-df[df$N==N&df$n==n&df$vr_int==vr_int&df$mult==mult&df$rho==rh&df$method%in%c(meth,"ml"),c(1:7,(22:25)+3)]
...
pdf("bin/figs/supp_bin_boundary.pdf",height=10,width=10)
...
pdf("bin/figs/supp_bin_loss.pdf",height=6,width=12)
ploss
dev.off()
...
pdf("bin/figs/supp_bin_beta5.pdf",height=10,width=10)
...
pdf("bin/figs/main_bin_boundary.pdf",height=6,width=12)
p0
dev.off()
